...
function quoteIdentifier(dialect, identifier, options) {
  ...
  switch (dialect) {
    ...
    case 'postgres':
      ...
      const [ literalAttribute, rawField ] = identifier;
      let field = Utils.removeTicks(rawField, '"');
      ...
      if (
        options.force !== true &&
          ...
          !identifier.includes('.') &&
          !identifier.includes('->') &&
          !postgresReservedWords.includes(field.toLowerCase())
      ) {
        field = Utils.addTicks(field, '"');
      }
